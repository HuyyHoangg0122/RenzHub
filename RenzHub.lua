...
local Config = {
    ...
    AutoTap = false, AutoUpgrade = false,
    AutoBuyTrail = false, AutoEquipBestTrail = false,
    AutoUpdate = false, AutoEquipBestPet = false, AutoPlaceEgg = false, AutoHatchEgg = false
}
...
Main.Size = UDim2.fromOffset(420, 1180)
Main.Position = UDim2.new(0.5, -210, 0.5, -590)
...
Section("⚡ AUTO TỔNG HỢP")
local AutoTapBtn = Button("👆 Auto Tập: OFF", function()
    Config.AutoTap = not Config.AutoTap
    AutoTapBtn.Text = "👆 Auto Tập: " .. (Config.AutoTap and "✅ ON" or "❌ OFF")
    AutoTapBtn.BackgroundColor3 = Config.AutoTap and Color3.fromRGB(30, 120, 60) or Color3.fromRGB(42, 42, 50)
end)

local AutoUpgradeBtn = Button("⬆️ Auto Nâng Cấp Máy: OFF", function()
    Config.AutoUpgrade = not Config.AutoUpgrade
    AutoUpgradeBtn.Text = "⬆️ Auto Nâng Cấp Máy: " .. (Config.AutoUpgrade and "✅ ON" or "❌ OFF")
    AutoUpgradeBtn.BackgroundColor3 = Config.AutoUpgrade and Color3.fromRGB(30, 120, 60) or Color3.fromRGB(42, 42, 50)
end)

local AutoBuyTrailBtn = Button("✨ Auto Mua Trail: OFF", function()
    Config.AutoBuyTrail = not Config.AutoBuyTrail
    AutoBuyTrailBtn.Text = "✨ Auto Mua Trail: " .. (Config.AutoBuyTrail and "✅ ON" or "❌ OFF")
    AutoBuyTrailBtn.BackgroundColor3 = Config.AutoBuyTrail and Color3.fromRGB(30, 120, 60) or Color3.fromRGB(42, 42, 50)
end)

local AutoEquipTrailBtn = Button("✨ Auto Trail Xịn Nhất: OFF", function()
    Config.AutoEquipBestTrail = not Config.AutoEquipBestTrail
    AutoEquipTrailBtn.Text = "✨ Auto Trail Xịn Nhất: " .. (Config.AutoEquipBestTrail and "✅ ON" or "❌ OFF")
    AutoEquipTrailBtn.BackgroundColor3 = Config.AutoEquipBestTrail and Color3.fromRGB(30, 120, 60) or Color3.fromRGB(42, 42, 50)
end)

local AutoUpdateBtn = Button("🔄 Auto Update: OFF", function()
    Config.AutoUpdate = not Config.AutoUpdate
    AutoUpdateBtn.Text = "🔄 Auto Update: " .. (Config.AutoUpdate and "✅ ON" or "❌ OFF")
    AutoUpdateBtn.BackgroundColor3 = Config.AutoUpdate and Color3.fromRGB(30, 120, 60) or Color3.fromRGB(42, 42, 50)
end)

local AutoEquipPetBtn = Button("🐾 Auto Pet Xịn Nhất: OFF", function()
    Config.AutoEquipBestPet = not Config.AutoEquipBestPet
    AutoEquipPetBtn.Text = "🐾 Auto Pet Xịn Nhất: " .. (Config.AutoEquipBestPet and "✅ ON" or "❌ OFF")
    AutoEquipPetBtn.BackgroundColor3 = Config.AutoEquipBestPet and Color3.fromRGB(30, 120, 60) or Color3.fromRGB(42, 42, 50)
end)

local AutoPlaceEggBtn = Button("🥚 Auto Đặt Trứng: OFF", function()
    Config.AutoPlaceEgg = not Config.AutoPlaceEgg
    AutoPlaceEggBtn.Text = "🥚 Auto Đặt Trứng: " .. (Config.AutoPlaceEgg and "✅ ON" or "❌ OFF")
    AutoPlaceEggBtn.BackgroundColor3 = Config.AutoPlaceEgg and Color3.fromRGB(30, 120, 60) or Color3.fromRGB(42, 42, 50)
end)

local AutoHatchEggBtn = Button("🐣 Auto Nở Trứng: OFF", function()
    Config.AutoHatchEgg = not Config.AutoHatchEgg
    AutoHatchEggBtn.Text = "🐣 Auto Nở Trứng: " .. (Config.AutoHatchEgg and "✅ ON" or "❌ OFF")
    AutoHatchEggBtn.BackgroundColor3 = Config.AutoHatchEgg and Color3.fromRGB(30, 120, 60) or Color3.fromRGB(42, 42, 50)
end)
...
task.spawn(function()
    while task.wait(0.3) do
        ...
        if Config.AutoTap then
            pcall(function()
                local btn = workspace:FindFirstChild("TapButton", true) or Player.PlayerGui:FindFirstChild("TapButton", true)
                if btn and btn:IsA("GuiButton") then fireclickdetector(btn) end
            end)
        end
        if Config.AutoUpgrade then
            pcall(function()
                local btn = workspace:FindFirstChild("UpgradeButton", true) or Player.PlayerGui:FindFirstChild("UpgradeButton", true)
                if btn and btn:IsA("GuiButton") then fireclickdetector(btn) end
            end)
        end
        if Config.AutoBuyTrail then
            pcall(function()
                local btn = workspace:FindFirstChild("BuyTrail", true) or Player.PlayerGui:FindFirstChild("BuyTrail", true)
                if btn and btn:IsA("GuiButton") then fireclickdetector(btn) end
            end)
        end
        if Config.AutoEquipBestTrail then
            pcall(function()
                local btn = workspace:FindFirstChild("EquipBestTrail", true) or Player.PlayerGui:FindFirstChild("EquipBestTrail", true)
                if btn and btn:IsA("GuiButton") then fireclickdetector(btn) end
            end)
        end
        if Config.AutoUpdate then
            pcall(function()
                local btn = workspace:FindFirstChild("UpdateButton", true) or Player.PlayerGui:FindFirstChild("UpdateButton", true)
                if btn and btn:IsA("GuiButton") then fireclickdetector(btn) end
            end)
        end
        if Config.AutoEquipBestPet then
            pcall(function()
                local btn = workspace:FindFirstChild("EquipBestPet", true) or Player.PlayerGui:FindFirstChild("EquipBestPet", true)
                if btn and btn:IsA("GuiButton") then fireclickdetector(btn) end
            end)
        end
        if Config.AutoPlaceEgg then
            pcall(function()
                local btn = workspace:FindFirstChild("PlaceEgg", true) or Player.PlayerGui:FindFirstChild("PlaceEgg", true)
                if btn and btn:IsA("GuiButton") then fireclickdetector(btn) end
            end)
        end
        if Config.AutoHatchEgg then
            pcall(function()
                local btn = workspace:FindFirstChild("HatchEgg", true) or Player.PlayerGui:FindFirstChild("HatchEgg", true)
                if btn and btn:IsA("GuiButton") then fireclickdetector(btn) end
            end)
        end
    end
